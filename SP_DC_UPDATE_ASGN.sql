USE [CDM_DEV2]
GO
/****** Object:  StoredProcedure [Census].[SP_DC_UPDATE_INTV_LOG]    Script Date: 30/01/2026 10:20:59 AM ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN TRY
		...
		DECLARE @sAsgnMainJson nvarchar(max) = (
			SELECT asgn.[ASGN_UID],
					'U' [RecordState]
			FROM #temp 
			INNER JOIN [Census].[CM_INTV_LOG] intvLog  WITH (NOLOCK) ON intvLog.[INTV_LOG_UID] = #temp.[INTV_LOG_UID]
